...
warming day value = (tmax-tmin/2)-lower
...
#test
wdd1 <- get_WDD(all_temps$tmax[1], all_temps$tmin[1], all_temps$temp[1])
#wdd_all <- get_WDD(all_temps$tmax, all_temps$tmin, all_temps$temp) #error
#wdd_all <- map(all_temps, get_WDD(all_temps, .$tmax, .$tmin, .$temp, upper=30, lower=3)) #%>%
#all_temps <- all_temps %>% mutate(wdd <- map(all_temps, get_WDD(tmax, tmin, temp)))
...
all_DDs <- all_temps %>% add_degree_days()
